...
def validate_smart_order(order_data: dict[str, Any]) -> tuple[bool, Optional[str]]:
    """Validate the incoming smart order data against required fields and valid options.

    Checks for:
    1. Presence of mandatory fields (symbol, quantity, action, etc.)
    2. Valid exchange (NSE, BSE, etc.)
    3. Valid action (BUY, SELL)
    4. Valid price type (LIMIT, MARKET, etc.)
    5. Valid product type (MIS, NRML, etc.)

    Args:
        order_data (dict[str, Any]): The order parameters to validate.

    Returns:
        tuple[bool, Optional[str]]: A tuple containing a success flag (bool)
            and an optional error message (str) if validation failed.
    """
...
def place_smart_order_with_auth(
    ...
    """Execute a smart order placement after authentication is verified.

    This is the core logic for smart orders. It handles:
    1. Validation of the order data.
    2. Routing to Sandbox Mode for testing if requested.
    3. Dynamic loading of the broker-specific API module.
    4. Handling 'No action needed' cases (when position matches target).
    5. Logging the order to the database.
    6. Sending notifications via WebSockets and Telegram.

    Args:
        order_data (dict[str, Any]): The processed smart order parameters.
        auth_token (str): Broker-specific session token.
        broker (str): The name of the broker (lowercase).
        original_data (dict[str, Any]): The raw request data for logging purposes.
        smart_order_delay (str): Delay in seconds before returning response
            (defaults to 0.5s).

    Returns:
        tuple[bool, dict[str, Any], int]: A tuple containing the success status,
            the response dictionary, and the HTTP status code.
    """
    ...
    try:
        ...
        logger.exception("Error in broker_module.place_smartorder_api")
        error_response = {
            "status": "error",
